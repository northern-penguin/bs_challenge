...
csv_file_dir = '/mnt/workspace/bs_challenge/app/intermediate/AB01_question_with_related_text_ot_normalized.csv'
q_file =  pd.read_csv(csv_file_dir,delimiter = ",",header = 0)

csv_file_dir_2 = '/mnt/workspace/bs_challenge/app/intermediate/AB01_question_with_related_text_rp.csv'
...
# 如果用的是原生的14B模型，use bf16
model = AutoModelForCausalLM.from_pretrained(model_dir, device_map="cuda:0", trust_remote_code=True).eval()

# int4版本的qwen需要提高temperature到0.5以上，否则报错RuntimeError: probability tensor contains either `inf`, `nan` or element < 0
model.generation_config = GenerationConfig.from_pretrained(model_dir, trust_remote_code=True, temperature = 0.6, top_p = 1, seed = 1234)
...
g = open('/mnt/workspace/bs_challenge/app/intermediate/FA_V5_Text_cap4_4_nt.csv', 'w', newline='', encoding = 'utf-8-sig') 
